...
| 2026-08-30 | `e74e413` | **Mobile responsiveness pass.** `app.css`: POS cart moved ABOVE product shelves on mobile (`.pos-grid`→flex column, `.cart{order:-1}`) so cashier can check out without scrolling past all products; product grid flows naturally (`max-height:none`, no nested scroll); `ops-section-head` stacks full-width buttons/headers on ≤768px; modals fill width with `flex:1` action buttons; KPIs/tiles tuned for 2-col mobile; scanbar wraps full-width on ≤420px. `index.php`: viewport meta relaxed (`width=device-width, initial-scale=1.0` — removed `maximum-scale=1, user-scalable=no`) so users can pinch-zoom (accessibility; inputs already ≥16px so no iOS auto-zoom). Verified live: CSS + meta served. | ✅ VPS |
