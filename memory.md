...
| 2026-08-30 | `a2df086` | **Stock badge text white on all states.** `app.css`: `.stockbadge.ok/.low` switched from tint backgrounds + dark text to solid `--lime`/`--coral` backgrounds with **`color:#fff`** (POS product cards). | ✅ VPS |
| 2026-08-30 | `785485f` | **POS cart visibility at 1042×727 + change discovery.** New `@media (max-width:1200px)` shrinks sidebar 240→200px (compact brand/nav, `overflow-y:auto`) and balances `.pos-grid` to `1.1fr 1fr` so the cart (change calc + Sell) shows full. Tablet `≤1024px` collapse now uses flex-column with `.pos-grid .cart{order:-1}` so cart sits ABOVE products (consistent with mobile). Change calculator stays in the cart under paymethods: select **Cash**, type amount given, read "Change to give the customer" (lime) / "Still owed" (coral). | ✅ VPS |
| 2026-08-30 | `c84ab92` | **Mobile-friendly product cards.** `app.css` `≤768px`: `.product-grid` now **single-column full-width cards** (was cramped `repeat(2,1fr)` ~150px) with padding 14px, larger `p-name` 16px, `p-fits` 13px `overflow-wrap:break-word`, price 18px, thumbs 110px. `≤420px` also forced to `1fr` (was `1fr 1fr` override). Fixes price+44px-add-button row overflow and long "fits" text on narrow 2-col cards; POS now fits viewport down to small phones. | ✅ VPS |
| 2026-08-30 | `8e8bc0a` | **Collapsible sidebar (desktop).** `index.php` sidebar brand row gains `#btnSidebarToggle` (`«`/`»`). `app.js`: `toggleSidebar()` toggles `sidebar-collapsed` on `#appShell`, persisted in `localStorage` (`ss_sidebar_collapsed`), restored on `init()`. `app.css` `≥769px`: collapsed = slim 64px icon rail (brand mark + nav icons + avatar only; labels/branch/user text hidden, `«`→`»`), nav flush to content (short screens never hide tabs), and `.pos-grid` widened to `1.5fr 1fr` so the **change calculator + Sell** fit fully at 1042×727. Mobile drawer (≤768) unaffected (collapsed rules live inside `min-width:769px`). | ✅ VPS |
| 2026-08-30 | `f5deeb7` | **Cache-busting (ROOT CAUSE of "styling not taking effect").** `assets/css/app.css` was the ONLY stylesheet (index.php:16), but loaded with **no `?v=` version** → browsers aggressively cached the old CSS/JS, so new rules (sidebar collapse, mobile product-grid, etc.) never appeared after deploy. Fix: each `<link>`/`<script src>` in `index.php` now appends `?v=<?= @filemtime(...) ?>` so the URL changes automatically whenever a CSS/JS file changes on disk — every deploy now forces browsers to fetch fresh styles. Verified live: served HTML shows `app.css?v=1788124275`. | ✅ VPS |
| 2026-08-30 | `decc306` | **Nicer collapse + 3-across product tiles.** Sidebar: brand label wrapped in `<span class="brand-label">` (was a bare text node CSS couldn't hide → text leaked/overflowed the 68px rail = "not nice"). Collapsed now = clean centered `S` mark + larger 48px rounded icon buttons, floating `»` expand button at rail edge; expanded = full labeled bar + `«` toggle. Products: medium `≤1200px` grid forced to `repeat(3,1fr)` with compact tiles (thumb 64px, tighter padding/type) so the shelf is shorter and the **Sales cart isn't squeezed into a corner**; combined with collapsed rail (products col ≈ 576px at 1042w → ~186px tiles). Mobile single-column unaffected (later rules override). | ✅ VPS |
| 2026-08-30 | `84c9d44` | **Collapse shows icons only + tiles exactly 3-across.** (1) Nav labels were bare text nodes inside `<button>` (app.js `buildNav`), so CSS couldn't hide them in collapsed mode → "Dashboard" text leaked over the icon rail. Fixed: each nav label wrapped in `<span class="nav-label">` (plus `title` for tooltip); collapsed CSS `display:none` on `.nav-label` → rail shows **only icons**. (2) Product grid base rule was `auto-fill minmax(180px,1fr)` → on desktops >1200px it rendered **5-across** (user saw `a b c d e / f g h`). Changed base `.product-grid` to `repeat(3,1fr)` so desktop is always **3-across** (`a b c / d e f / g h`); medium/tablet/mobile overrides intact. | ✅ VPS |
| 2026-08-30 | `e48dcec` | **POS products shelf thinner; cart gets majority width.** Product column was too wide (base `1.6fr 1fr`, collapsed `1.5fr 1fr`, medium `1.1fr 1fr`) → Sales/cart div couldn't sit on the same page as products. Changed ratios so **cart > products**: base `1fr 1.15fr`, collapsed `1fr 1.2fr`, medium `1fr 1.15fr` (gap 12). At 1042×727 (expanded 200px sidebar): products ≈386px (3-across ≈123px/card), cart ≈444px. Collapsed rail: products ≈434px, cart ≈520px. Keeps `a b c` 3-across and fits cart + change calc + Sell on one page. | ✅ VPS |
| 2026-08-30 | `3c74b0e` | **POS products column clearly thinner (user: "shorter not thin").** Raised ratios further so the **cart is the clear majority (~62%)** and products are visibly thin, keeping 3-across: base `1fr 1.8fr`, collapsed `1fr 1.7fr`, medium `1fr 1.6fr`. At 1042×727 expanded: products ≈319px (3-across ≈101px/card), cart ≈511px. User explicitly chose "keep 3-across, thin column" — accepts cramped ~100px cards to keep the Sales div on the same page. | ✅ VPS |
...
| 2026-08-31 | (PV) | **PV DESIGN implemented + product-picker.** (1) Appended "PV DESIGN" override block to `app.css`: fixed-height shell (`#appShell .shell-flex/.sidebar/.main` height:100vh, main scrolls internally); sidebar collapse redesigned to pv.html ─ collapsed = 72px rail hiding `.brand-label/.nav-label/.helpline/branch/user-text`, plus a separate `»` `.sidebar-toggle-collapsed` expand button below the brand (e.g. open shows `«`); green `.helpline` card added; `.product-card` radius 14 + shadow; `.btn-add-cart` now accent-filled with "+ Add"/"Out" text; two-row `.cart-line` (name full row, qty-controls+`line-total` second row) inside `.cart-scroll` (sticky full-height cart); `.product-pager` (‹ status ›) for mobile; responsive 1280/1024/768/420 (tablet sidebar becomes drawer, cart `order:-1` on top). Cache-busting already live so CSS/JS refresh on deploy. (2) "Add Custom Item" → **searchable product picker**: `productPickerModal` w/ search input + scrollable `.picker-row` list showing each part's **available stock**; clicking selects → shows SKU + `N available`, qty input **capped at stock** (max=stock-already-in-cart), price auto-filled from catalog (editable), Add → `POS.pickerAdd()` merges as a **normal catalog line** (id+qty, deducts stock on checkout) not a MANUAL line. `addToCart(id, qty=1)` and `updateQty` now cap qty at branch stock. Complete button reads `Add items to continue` / `Complete Sale — Collect GHS X`. Old manual-modal elements removed (no stale refs). `preview.html` remains as user-approved design preview. Commit `4f05a56`. | v. VPS |
